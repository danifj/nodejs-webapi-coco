...
ChildrenRepository.prototype = {
  save: function (e, cb, errCb) {
    ...
    setChild: function (e, cb, errCb) {
        let db = this.db;
        let data = [e.firstName,e.lastName,e.age,e.birthdate,e.dni,e.diseases,e.allergies,e.phone1,e.phone2,e.phone3,e.street,e.streetNumber,e.floor,e.department,e.neighborhood,e.city,e.fatherFirstName,e.fatherLastName,e.fatherEmail,e.motherFirstName,e.motherLastName,e.motherEmail,e.roomId,e.turnId,e.authorizedPersons,e.id];
    ...
    getChildrenByRoomId: function(id, cb, errCb){
        ...
        db.query(query, [id], (err, results, fields) => {
            ...
            for(let i=0;i<results[0].length;i++){
                let e = results[0][i];
                let child = new Child(e.CHILD_ID,e.FIRST_NAME,e.LAST_NAME,e.AGE,e.BIRTHDATE,e.DNI,e.DISEASES,e.ALLERGIES,e.PHONE1,e.PHONE2,e.PHONE3,e.STREET,e.STREET_NUMBER,e.FLOOR,e.DEPARTMENT,e.NEIGHBORHOOD,e.CITY,e.FATHER_FIRST_NAME,e.FATHER_LAST_NAME,e.FATHER_EMAIL,e.MOTHER_FIRST_NAME,e.MOTHER_LAST_NAME,e.MOTHER_EMAIL,e.ROOM_ID,e.TURN_ID,e.AUTHORIZED_PERSONS);
                children.push(child);
            }
            cb(children);
        });
